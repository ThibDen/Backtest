...
def calculate_values(months, Data,Tot_fact_exposure,start_cap, monthly_cap, monthly_correction,Net_of_costs):  
    ...
    Value_fixed_inv = np.zeros((time_span_data-months ,months)) #simulate one fixed investment at begin of the period
    ...
    for k in range( time_span_data-months): #start simulation
        ...
        for i in range(1,months): # iterate over the number of months
            Value[k,i]  = Value[k,i-1] *returns[i+k] +monthly_cap*Net_of_costs
            Value_fixed_inv[k,i]  = Value_fixed_inv[k,i-1] *returns[i+k] #current value of etf's is last months value of etf's * monthly returns
...
def create_histogram_gif(Returns_A, Returns_B, Start_invest, Monthly_DCA, portfolio_name_A, portfolio_name_B):
    ...
    # Define the bins for the histogram
        bins = np.linspace(min(np.min(Returns_A),np.min(Returns_B)), max(np.max(Returns_A),np.max(Returns_B)), 50)
    else:
        bins = np.linspace(0.7,1.40, 50)
